...
import { User, UserRole, LogType } from '../types';
...
const GestaoUtilizadores: React.FC<GestaoUtilizadoresProps> = ({ onAddLog }) => {
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    let confirmMessage = "Tem a certeza que deseja apagar este utilizador?";

    if (userToDelete?.role === 'Cliente') {
      confirmMessage = `ATENÇÃO: Apagar o cliente "${userToDelete.name}" eliminará também TODOS os pedidos de suporte, mensagens e históricos associados.\n\nEsta ação é irreversível.\n\nDeseja confirmar?`;
    }
    ...
    try {
      ...
      onAddLog('Remoção de Utilizador', `Utilizador ID ${userId} (${userToDelete?.name}) foi removido`, LogType.SYSTEM);
    } catch (error) {
      console.error("Erro ao apagar:", error);
      alert("Erro ao apagar utilizador. Verifique se existem dependências ou tente novamente.");
    }
  };
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onAddLog('Criação de Utilizador', `Novo utilizador criado: ${newUser.name} (${newUser.role})`, LogType.SYSTEM);
