...
const PainelPrincipal: React.FC = () => {
  ...
  const processActivityData = (activityData: any[]) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const existing = activityData?.find((item: any) => item.date === dateStr);

      // Format day name (Seg, Ter, etc)
  ...
  const chartData = processActivityData(stats.recentActivity);
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2.5">
                <div className="bg-amber-500 h-2.5 rounded-full" style={{ width: `${stats.totalTickets ? (stats.pendingTickets / stats.totalTickets) * 100 : 0}%` }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2.5">
                <div className="bg-indigo-500 h-2.5 rounded-full" style={{ width: `${stats.totalTickets ? (stats.activeTickets / stats.totalTickets) * 100 : 0}%` }}></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-2.5">
                <div className="bg-emerald-500 h-2.5 rounded-full" style={{ width: `${stats.totalTickets ? (stats.completedTickets / stats.totalTickets) * 100 : 0}%` }}></div>
              </div>
            </div>
          </div>
        </div>
